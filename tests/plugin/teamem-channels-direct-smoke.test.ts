...
const channelsSmokeCases: readonly ChannelsSmokeCase[] = [
  {
    caseName: 'direct',
    description:
      'delivers Alice direct slash-command message to Bob through Channels without Alice or Carol echo',
    lockName: 'teamem-channels-direct',
    expectedDeliveryScope: 'direct',
    expectedRecipientPrincipals: (runtimes) => [runtimes.bob.whoami.principal],
    promptArgs: ({ body, runtimes }) =>
      `${runtimes.bob.whoami.principal} -- ${body}`
  },
  {
    caseName: 'star',
    description:
      'delivers Alice * slash-command broadcast to Bob and Carol through Channels without Alice echo',
    lockName: 'teamem-channels-star',
  ...
  },
  {
    ...
    description:
      'delivers Alice ** slash-command broadcast to Bob and Carol through Channels without Alice echo',
    lockName: 'teamem-channels-starstar',
    expectedDeliveryScope: 'space',
    expectedRecipientPrincipals: () => [],
    promptArgs: ({ body }) => `** -- ${body}`
  }
...
describeLiveChannelsDirect(
  `Teamem L5 Channels live smoke${gate.enabled ? '' : ` (${gate.reason})`}`,
  () => {
...
describe('Teamem Channels direct smoke evidence waits', () => {
...
async function runChannelsLiveCase(
  ...
  try {
    ...
    expect(post.evidence.sprint_id).toBeNull();
    expect(post.evidence.recipient_principals).toEqual(
      smokeCase.expectedRecipientPrincipals(runtimeByPersona)
    );

    const deliveryMatrix = expectedTeamemChannelsDeliveryMatrix(
      smokeCase.caseName
    );
    const directTargetPrincipal = bobRuntime.whoami.principal;
    const expectations = {
      alice: createExpectedReceipt({
        ...
        recipientPrincipal:
          smokeCase.caseName === 'direct'
            ? directTargetPrincipal
  ...
  while (Date.now() < deadline) {
    ...
    try {
      ...
      throw new PostMessageEvidenceError(
        `matched post_message response for marker ${input.marker} but delivery route did not match expected scope=${input.expectedDeliveryScope} recipients=${JSON.stringify(input.expectedRecipientPrincipals)}. Observed scope=${evidence.delivery_scope} recipients=${JSON.stringify(evidence.recipient_principals)}. Artifacts: ${input.session.artifacts.dir}`,
        { transient: false }
      );
    } catch (error) {
      lastError = error;
...
async function writeEvidenceSnapshot(
  ...
  const prefix = `${launchedPersona.personaPlan.persona}-channels-direct-${launchedPersona.runtime.sessionId}`;
  const traces = await readMcpTraces(
    launchedPersona.session.artifacts.mcpTraceDir,
    {
      ignoreTransientErrors: true
...
function assertNoReadThreadCall(input: {
  ...
  if (readThreadCall) {
    throw new Error(
      `command/MCP post: read_thread must not be used by Channels direct smoke. Artifacts: ${input.artifactsDir}`
    );
  }
}
