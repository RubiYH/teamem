...
| `teamem-flag` | Manages per-session activation state (enable/disable/status); controls active flag and monitor lifecycle |
...
- **Bridge invocation model**: Each script routes to `bun run ${PLUGIN_ROOT}/lib/bridge.js` with different argument modes:
  ...
  - `teamem-flag`: reads/writes session state files (`.active`, `.space`, `.persist-flag`), controls monitor PID
...
- **teamem-flag state**: Session state lives in `${CLAUDE_PLUGIN_DATA}/sessions/${SID}/` with files: `active` (timestamp of activation), `space` (pinned space name), `monitor.pid` (background process). Project-wide persistence writes to `.claude/plugin-data/teamem/auto-on`.
