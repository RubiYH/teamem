...
Plugin manifest and metadata for the Claude Code marketplace. Contains the single source-of-truth plugin definition (`plugin.json`) that declares the plugin name, version, description, explicit command and agent component lists, skills path, and MCP server configuration.
