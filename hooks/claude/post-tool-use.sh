#!/usr/bin/env bash
# teamem — Claude Code PostToolUse hook
# Publishes a task_progressed event after tool execution. Advisory only — always exits 0.
...
TEAMEM_TOOL_NAME="${TEAMEM_TOOL_NAME:-unknown}"  # name of the Claude tool that just ran
...
BODY=$(printf '{"repo_id":"%s","principal":"%s","actor":"%s","delegation":"%s","event_type":"task_progressed","scope":{},"payload":{"task_id":"session-%s","what":"used tool: %s"}}' \
  "$TEAMEM_REPO_ID" "$TEAMEM_PRINCIPAL" "$TEAMEM_ACTOR" "$TEAMEM_DELEGATION" \
  "$SESSION_ID" "$TEAMEM_TOOL_NAME")
